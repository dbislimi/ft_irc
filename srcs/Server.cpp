...
/*   Updated: 2025/03/17 11:26:34 by bsafi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::map<int, Client *>::iterator it = _clients.find(fd);
	std::cout << fd << "  Boolname: " << it->second->getBoolName() << std::endl;
	if (it->second->getBoolName() == false)
	{
		...
		for (std::map<int, Client *>::iterator it = _clients.begin(); it != _clients.end(); ++it)
		{
			if (_cmd[0] == it->second->getNickName())
			{
				it->second->setBoolName(false);
				ask = "Nickname already taken\r\t";
				send(fd, ask.c_str(), ask.length(), 0);
				break;
			}
			else
