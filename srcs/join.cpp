...
void Server::joinChannel(std::string channel, int fd)
{
	...
	if (!_channels[channel]->getTopic().empty())
		mysend(fd, ":" + _name + " 332 " + _clients[fd]->getNickName() +  " " + channel + _channels[channel]->getTopic() +"\r\n");
	...
	mysend(fd, msg + "\r\n");
	mysend(fd, ":" + _name + " 366 " + _clients[fd]->getNickName() +  " " + channel + " :End of /NAMES list\r\n");
}
...
void Server::JOIN(int fd, std::deque<std::string> cmd)
{
	...
	for (size_t	i = 0; i < channels.size(); ++i){
		if ((*(channels.begin() + i))[0] != '#')
			mysend(fd, ":" + _name + " 403 " + _clients[fd]->getNickName() +  " " + *(channels.begin() + i) + " :No such channel\r\n");
		else{
			...
			if (_channels[*(channels.begin() + i)]->getIsmdp() == true && (cmd.size() <= 2 || i >= keys.size() || *(keys.begin() + i) != _channels[*(channels.begin() + i)]->getMdp())){
				mysend(fd, ":" + _name + " 475 " + _clients[fd]->getNickName() +  " " + *(channels.begin() + i) + " :Cannot join channel (+k)\r\n");
				continue ;
			}
			if (_channels[*(channels.begin() + i)]->getisLimitUser() == true && _nbCliChannel[*(channels.begin() + i)].size() >= static_cast<size_t>(_channels[*(channels.begin() + i)]->getLimitUser())){
				mysend(fd, ":" + _name + " 471 " + _clients[fd]->getNickName() +  " " + *(channels.begin() + i) + " :Cannot join channel (+l)\r\n");
				continue ;
			}
